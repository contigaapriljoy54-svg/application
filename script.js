...
photoInput.addEventListener('change', () => {
  ...
  if (!file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) {
    clearPhoto();
    photoError.textContent = 'Choose a JPG, PNG, or WebP image under 5 MB.';
    return;
  }
  if (photoUrl) URL.revokeObjectURL(photoUrl);
  photoUrl = URL.createObjectURL(file);
  photoPreview.innerHTML = `<img src="${photoUrl}" alt="Selected portrait preview">`;
  photoPreview.setAttribute('aria-label', 'Selected photograph preview');
...
window.addEventListener('beforeunload', () => { if (photoUrl) URL.revokeObjectURL(photoUrl); });
